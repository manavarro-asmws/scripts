            ...
            if (Test-Path $logFile) {
                try {
                    ...
                    $resumenLogs.Add([pscustomobject]@{
                        ...
                        EjemploUltimoError  = if ($errores.Count -gt 0) { ($errores[-1].Line) -replace '<!\[LOG\[','' -replace '\]LOG\]!>.*','' } else { '' }
                    })
                }
                catch {
...
$erroresHtml = if ($script:Errores.Count -gt 0) {
    "<h2>Avisos durante la ejecucion</h2><ul>" + (($script:Errores | ForEach-Object { "<li>$_</li>" }) -join "`n") + "</ul>"
} else { "<p>Sin errores durante la ejecucion.</p>" }
