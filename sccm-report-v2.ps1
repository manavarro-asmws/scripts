<#
================================================================================
 Get-SCCMAssessmentData.ps1
================================================================================
 PROPOSITO
   Script de apoyo al "Assessment de Salud e Infraestructura" de Configuration
   Manager (ARAG). Recopila, de forma puramente informativa, datos que no son
   visibles (o son poco fiables) desde capturas de pantalla de la consola:
   tamaño y estado de la base de datos SQL, espacio en disco, fragmentacion de
   indices, historial de backups, cumplimiento agregado de actualizaciones,
   errores recientes en logs, certificados proximos a caducar, etc.

 GARANTIA DE SOLO LECTURA (LEELO ANTES DE EJECUTAR)
   - El script NUNCA ejecuta Set-*, New-*, Remove-*, Update-*, Invoke-CM*,
     ni ningun cmdlet del modulo de Configuration Manager que pueda escribir.
   - Todo el acceso a Configuration Manager se hace contra la base de datos de
     SQL Server, usando EXCLUSIVAMENTE las vistas "v_*" documentadas por
     Microsoft para generacion de informes (el mismo mecanismo que usan SSRS
     y Power BI Report Server) mediante sentencias SELECT.
   - Toda consulta SQL pasa por Invoke-ReadOnlyQuery, que rechaza cualquier
     texto que no empiece literalmente por SELECT o WITH (CTE de solo
     lectura). Es una salvaguarda adicional, no solo una convencion.
   - El acceso a archivos (logs, Content Library) es solo lectura
     (Get-Content, Get-ChildItem); no se borra, mueve ni modifica ningun
     archivo.
   - Idealmente, ejecuta este script con una cuenta con permisos de solo
     lectura sobre la base de datos (rol db_datareader sobre CM_<SiteCode> y
     sobre msdb), y con un usuario que solo tenga "Read-only Analyst" en la
     consola de Configuration Manager si en algun momento se amplia el
     script para leer via WMI.
   - La fragmentacion de indices usa el modo LIMITED de
     sys.dm_db_index_physical_stats (barato en I/O). No se ejecuta el modo
     DETAILED, que si puede ser costoso en bases de datos grandes.

 REQUISITOS
   - PowerShell 5.1+ ejecutado en (o con conectividad a) el servidor de
     sitio / servidor SQL.
   - Modulo "SqlServer" (o el antiguo "SQLPS") para Invoke-Sqlcmd. Si no esta
     disponible, el script usa un fallback nativo via System.Data.SqlClient,
     tambien de solo lectura.
   - Cuenta con permisos de lectura sobre la base de datos del sitio
     (CM_<SiteCode>) y, opcionalmente, sobre msdb (historial de backups y
     de SQL Agent) para las secciones correspondientes.

 USO
   .\Get-SCCMAssessmentData.ps1
   .\Get-SCCMAssessmentData.ps1 -SiteServer SERVERSCCM01.arag.local -SiteCode ESP -SqlInstance SERVERSCCM01 -Database CM_ESP
   .\Get-SCCMAssessmentData.ps1 -OutputPath D:\Assessment -Sections Sql,Disco,Logs
   .\Get-SCCMAssessmentData.ps1 -ListSections

 SALIDA
   Un informe HTML con todas las secciones y, ademas, un CSV por seccion en
   una subcarpeta "csv", para poder adjuntarlos o pegarlos directamente en
   el documento Word del assessment.
================================================================================
#>
...
function Invoke-ReadOnlyQuery {
    ...
    try {
        ...
        if (Get-Command Invoke-Sqlcmd -ErrorAction SilentlyContinue) {
            ...
            $connString = "Server=$Instance;Database=$Db;Integrated Security=True;TrustServerCertificate=True;Connection Timeout=30;"
            if ($SqlCredential) {
                $connString = "Server=$Instance;Database=$Db;User Id=$($SqlCredential.UserName);Password=$($SqlCredential.GetNetworkCredential().Password);TrustServerCertificate=True;Connection Timeout=30;"
            }
...
if (Test-SectionEnabled 'Sql') {
    ...
    try {
        ...
        Save-Resultado 'sql_version' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db 'master' -Descripcion 'Version SQL') 'Version y edicion de SQL Server'
...
"@
        Save-Resultado 'sql_ficheros' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db 'master' -Descripcion 'Tamano ficheros BD') 'Tamano y crecimiento de los ficheros de la base de datos'
        ...
        Save-Resultado 'sql_broker' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db 'master' -Descripcion 'Service Broker') 'Estado de Service Broker y modelo de recuperacion'
...
"@
        Save-Resultado 'sql_backups' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db 'msdb' -Descripcion 'Historial de backups') 'Ultimos 10 backups de la base de datos del sitio'
...
"@
        Save-Resultado 'sql_agent_jobs' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db 'msdb' -Descripcion 'SQL Agent jobs de CM') 'Historial de tareas de SQL Agent relacionadas con Configuration Manager'
...
"@
        Save-Resultado 'sql_fragmentacion' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db $Database -Descripcion 'Fragmentacion de indices') 'Top 25 indices mas fragmentados (modo LIMITED)'
...
ORDER BY Status
"@
        Save-Resultado 'sql_update_compliance' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db $Database -Descripcion 'Compliance de actualizaciones') 'Resumen agregado de v_UpdateComplianceStatus (contraste con el Software Updates Dashboard)'
...
FROM v_AuthListInfo
"@
        Save-Resultado 'sql_update_groups' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db $Database -Descripcion 'Recuento Software Update Groups') 'Recuento real de Software Update Groups (total / vacios / generados por la ADR de Defender)'
...
"@
        Save-Resultado 'sql_client_health' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db $Database -Descripcion 'Client Health summary') 'Resumen agregado de salud de cliente (v_CH_ClientSummary)'
...
"@
        Save-Resultado 'sql_status_messages' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db $Database -Descripcion 'Status messages') "Mensajes de Error/Warning por componente (ultimos $DiasStatusMessages dias)"
...
"@
        Save-Resultado 'sql_content_grande' (Invoke-ReadOnlyQuery -Query $q -Instance $SqlInstance -Db $Database -Descripcion 'Paquetes mas grandes') 'Top 20 paquetes/contenido por tamano (candidatos a limpieza si estan obsoletos)'
...
if (Test-SectionEnabled 'Disco') {
    ...
    try {
        $discos = Get-CimInstance -ComputerName $SiteServer -ClassName Win32_LogicalDisk -Filter "DriveType=3" |
        ...
        Write-Log "Error en la seccion Disco: $($_.Exception.Message)" 'ERR'
        $script:Errores.Add("Seccion Disco: $($_.Exception.Message)")
    }
}
...
if (Test-SectionEnabled 'Logs') {
    ...
    try {
        ...
        Write-Log "Error en la seccion Logs: $($_.Exception.Message)" 'ERR'
        $script:Errores.Add("Seccion Logs: $($_.Exception.Message)")
    }
}
...
if (Test-SectionEnabled 'Certificados') {
    ...
    try {
        ...
        Write-Log "No se pudo leer el almacen de certificados remoto (¿WinRM deshabilitado?): $($_.Exception.Message)" 'WARN'
        $script:Errores.Add("Seccion Certificados: $($_.Exception.Message)")
    }
}
